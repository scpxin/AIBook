...
| M13 | plot_nodes | 剧情节点 | v2_plot_nodes |
| M14 | scene_design | 场景设计 | v2_scenes |
| M15 | draft_generation | 正文生成 | v2_drafts |
| M16 | content_parsing | 内容解析 | v2_pipeline_states |
| M17 | polish | 润色 | v2_pipeline_states |
| M18 | knowledge_update | 知识库 | v2_knowledge_states |
| M19 | consistency_check | 一致性检查 | v2_consistency_reports |
...
### AI生成
- `POST /api/v2/ideas/generate` - 生成灵感
- `POST /api/v2/world/origin` - 生成世界观
- `POST /api/v2/characters/protagonist` - 生成主角
- `POST /api/v2/projects/analyze` - 分析项目定位
- `POST /api/v2/story/master` - 生成故事框架
- `POST /api/v2/power-system/generate` - 生成力量体系
- `POST /api/v2/factions/generate` - 生成势力
- `POST /api/v2/timeline/build` - 构建时间线
- `POST /api/v2/volumes/generate` - 生成卷纲
- `POST /api/v2/chapters/plan` - 章节规划
- `POST /api/v2/chapters/outline` - 章节细纲
- `POST /api/v2/plot-nodes/generate` - 剧情节点
- `POST /api/v2/scenes/design` - 场景设计
- `POST /api/v2/draft/generate` - 生成正文
- `POST /api/v2/polish` - 润色
- `POST /api/v2/knowledge/update` - 知识更新
