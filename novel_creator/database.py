...
def _migrate(conn):
    ...
    if ver < 2:
        ...
        for sql in [
            ...
            "ALTER TABLE step_summaries ADD COLUMN deleted_at TEXT DEFAULT NULL",
            "ALTER TABLE generation_status ADD COLUMN deleted_at TEXT DEFAULT NULL",
            "ALTER TABLE outlines ADD COLUMN deleted_at TEXT DEFAULT NULL",
            "ALTER TABLE outline_generation_status ADD COLUMN deleted_at TEXT DEFAULT NULL",
...
def list_projects(include_archived=False, include_deleted=False):
    ...
    with _db_lock:
        ...
        if not include_deleted and _has_column('projects', 'deleted_at'):
...
def delete_project_cascade(project_id):
    ...
    with _db_lock:
        conn = get_db()
        now = _now()
        conn.execute("UPDATE projects SET deleted_at=? WHERE id=?", (now, project_id))
        conn.execute("UPDATE step_summaries SET deleted_at=? WHERE project_id=?", (now, project_id))
        conn.execute("UPDATE chapters SET status='deleted' WHERE project_id=?", (project_id,))
        conn.execute("UPDATE generation_status SET deleted_at=? WHERE project_id=?", (now, project_id))
        conn.execute("UPDATE outlines SET deleted_at=? WHERE project_id=?", (now, project_id))
        conn.execute("UPDATE outline_generation_status SET deleted_at=? WHERE project_id=?", (now, project_id))
        conn.commit()
        conn.close()


def hard_delete_project(project_id):
    """硬删除项目及其所有关联数据（用于30天后的垃圾回收）"""
    with _db_lock:
        conn = get_db()
...
def restore_project(project_id):
    ...
    with _db_lock:
        ...
        conn.execute("UPDATE step_summaries SET deleted_at=NULL WHERE project_id=?", (project_id,))
        conn.execute("UPDATE chapters SET status='done' WHERE project_id=? AND status='deleted'", (project_id,))
        conn.execute("UPDATE generation_status SET deleted_at=NULL WHERE project_id=?", (project_id,))
        conn.execute("UPDATE outlines SET deleted_at=NULL WHERE project_id=?", (project_id,))
        conn.execute("UPDATE outline_generation_status SET deleted_at=NULL WHERE project_id=?", (project_id,))
        conn.commit()
        conn.close()
...
def get_chapter(project_id, chapter_number):
    ...
    with _db_lock:
        conn = get_db()
        row = conn.execute(
            "SELECT * FROM chapters WHERE project_id=? AND chapter_number=? AND status != 'deleted'",
...
def get_all_chapters(project_id):
    """获取项目所有章节（按章节号排序，排除已删除）"""
    with _db_lock:
        conn = get_db()
        rows = conn.execute(
            "SELECT * FROM chapters WHERE project_id=? AND status != 'deleted' ORDER BY chapter_number",
            (project_id,)
        ).fetchall()
        conn.close()
