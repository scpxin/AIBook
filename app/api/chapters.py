...
def chapter_save(body: ChapterSaveRequest):
...
def generation_start(body: dict):
    project_id = body.get('projectId', '')
...
def generation_pause(body: dict):
    project_id = body.get('projectId', '')
    if not project_id:
        return {"error": "缺少 projectId"}
...
def generation_update(body: dict):
    project_id = body.get('projectId', '')
