...
def inspiration_title(body: dict):
...
def inspiration_description(body: dict):
    cfg = validate_ai_config(body)
    if not cfg:
        return {"error": "缺少AI配置参数"}
    gen = get_generator(cfg['endpoint'], cfg['api_key'], cfg['model'], body.get('temperature', 0.7), body.get('maxTokens', 4000))
...
def inspiration_theme(body: dict):
    cfg = validate_ai_config(body)
    if not cfg:
        return {"error": "缺少AI配置参数"}
    gen = get_generator(cfg['endpoint'], cfg['api_key'], cfg['model'], body.get('temperature', 0.7), body.get('maxTokens', 4000))
...
def inspiration_genre(body: dict):
    cfg = validate_ai_config(body)
    if not cfg:
        return {"error": "缺少AI配置参数"}
    gen = get_generator(cfg['endpoint'], cfg['api_key'], cfg['model'], body.get('temperature', 0.7), body.get('maxTokens', 4000))
...
def worldbuilding_reparse(body: dict):
    cfg = validate_ai_config(body)
    if not cfg:
        return {"error": "缺少AI配置参数"}
    gen = get_generator(cfg['endpoint'], cfg['api_key'], cfg['model'], body.get('temperature', 0.7), body.get('maxTokens', 4000))
...
def characters_reparse(body: dict):
    cfg = validate_ai_config(body)
    if not cfg:
        return {"error": "缺少AI配置参数"}
    gen = get_generator(cfg['endpoint'], cfg['api_key'], cfg['model'], body.get('temperature', 0.7), body.get('maxTokens', 4000))
...
def chapter_summarize(body: ChapterSummarizeRequest):
...
def generate_chapter_stream(body: dict):
...
def novel_analyze_style(body: dict):
    cfg = validate_ai_config(body)
    if not cfg:
        return {"error": "缺少AI配置参数"}
