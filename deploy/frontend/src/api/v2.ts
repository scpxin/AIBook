...
import { apiPost, apiGet, apiStream } from './client'
import type {
...
export function generateIdeas(projectId: string, userInput: string, genreHint?: string, count?: number) {
  return apiPost<{ ideas: IdeaCandidate[]; projectId: string }>('/api/v2/ideas/generate', {
...
export function parseContent(projectId: string, chapterNo: string, content: string) {
