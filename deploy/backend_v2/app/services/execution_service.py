...
输出完整的正文内容(不分段),目标2000-3000字。"""
        ...
        try:
            ...
            for chunk in gen.client.generate_stream(prompt, temperature=0.8, max_tokens=8000):
                if chunk:
                    full_content += chunk
                    yield {"type": "chunk", "content": chunk, "full_length": len(full_content)}
...
原文: {content[:5000]}
润色焦点: {focus}
伏笔保护(不可删改): {json.dumps(foreshadow_protected or [], ensure_ascii=False)}

三阶段润色:
1. language_polish — 语言优化(消除语病/优化修辞/提升画面感/去除冗余)
2. rhythm_adjustment — 节奏调整(句式长短/段落分配/紧张度波动)
3. originality_check — 查重检测(与经典作品的相似度/套话检测/独创性评分)

返回JSON:
{{"polished_content": "润色后全文",
  "changes": [{{"type": "language/rhythm/originality", "position": "位置描述", "original": "原文", "new": "修改后", "reason": "修改原因"}}],
  "similarity_report": {{
    "overall_similarity": 0.15,
    "matches": [{{"source": "疑似来源", "passage": "文段", "similarity": 0.8, "suggestion": "改写建议"}}]
  }},
  "improvement_summary": "润色总结(200字)"}}"""
...
def _build_style_str(style_profile: dict = None) -> str:
    ...
    if not style_profile:
        return ""
    parts = []
    for key in ['narrative_perspective', 'tone', 'pacing']:
