    ...
    with _db_lock:
        ...
        conn.execute("""
            INSERT INTO outlines (project_id, chapter_number, title, summary, scenes, characters, key_points,
                emotion, goal, technique_focus, book_overview, acts, importance, status, error_message, metadata_json, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ON CONFLICT(project_id, chapter_number) DO UPDATE SET
                ...
                book_overview=excluded.book_overview, acts=excluded.acts, importance=excluded.importance,
        ...
        """, (project_id, chapter_number, title, summary, scenes_json, chars_json, kp_json,
              emotion, goal, technique_focus, book_overview, acts_json, importance, status, error_message, meta_str, _now()))
        conn.commit()
        conn.close()
