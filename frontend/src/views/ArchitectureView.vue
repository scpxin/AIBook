...
const gen = useGeneration('architecture', '故事架构')
...
async function confirm() {
  ...
  try {
    await saveModuleData(props.projectId, 'architecture', data)
...
onMounted(async () => {
  try {
    const saved = await getModuleData(props.projectId, 'architecture')
    if (saved?.data) {
      const d = saved.data
